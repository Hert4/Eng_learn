...
const VoiceAssistant = () => {
    ...
    const wsUrl = 'https://107.124.124.71:3558/ws/assistant';
        ...
        recognitionRef.current.onresult = (event) => {
            ...
            if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
                if (interimTranscript) {
                    wsRef.current.send(
                        JSON.stringify({ transcript: interimTranscript, is_final: false })
                    )
                }
                if (finalTranscript) {
                    wsRef.current.send(
                        JSON.stringify({ transcript: finalTranscript, is_final: true })
                    )
                }
            }
        };
